...
if [ "`lsb_release -d | sed 's/.*:\s*//' | sed 's/16\.04\.[0-9]/16.04/' `" != "Ubuntu 16.04 LTS" ]; then
	echo "PartCCTV only supports being installed on Ubuntu 16.04, sorry. You are running:"
	echo
...
if [ ! -d /vagrant ]; then
	...
	echo "PartCCTV needs more memory (RAM) to function properly."
...
if [ -n "$MOUNTED_TMP_AS_NO_EXEC" ]; then
	echo "Installer has to have exec rights on /tmp, please mount /tmp with exec"
	exit
fi

# Check that no .wgetrc exists
if [ -e ~/.wgetrc ]; then
	echo "Installer expects no overrides to wget defaults, ~/.wgetrc exists"
	exit
fi
...
if [ -z "$ARM" ]; then
	echo "PartCCTV only supports x86_64 or i686 and will not work on any other architecture, like ARM."
